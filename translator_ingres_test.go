...
func TestIngres(t *testing.T) {
	...
	for _, driver := range []string{"postgres", "pgx"} { // test 2 drivers différents
		...
		AssertSqlQuery(t, db, "select $1*$2", []string{"6.52"}, "0000000003.26", "2")
		...
		AssertSqlExec(t, db, true, "modify public.TABLE2 to isam on column2 with fillfactor = 80, extend = 16, page_size = 8192", 0)
