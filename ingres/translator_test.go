...
func TestTranslations(t *testing.T) {
	...
	defer func() {
		sqlutils.AssertNoError(t, postgres.Stop())
	}()
	...
	connectionsTest := func(name string, url string, profile bool) {
		...
		for range numThreads {
			wg.Go(func() {
				...
				sqlutils.AssertNoError(t, err)
				defer db.Close()
				sqlutils.AssertSqlQuery(t, db, "SELECT pg_sleep("+latency+")", []string{""})
				//})
			})
		}
		wg.Wait()
	...
	for _, driver := range []string{"postgres", "pgx"} { // test 2 drivers différents
		...
		sqlutils.AssertSqlQuery(t, db, "SELECT 'a' + ?", []string{"ab"}, "b")
		sqlutils.AssertSqlQuery(t, db, "SELECT 'a' + ?1", []string{"ab"}, "b")
		sqlutils.AssertSqlQuery(t, db, "SELECT 'a' + @1", []string{"ab"}, "b")
		sqlutils.AssertSqlQuery(t, db, "SELECT 'a' + :1", []string{"ab"}, "b")
		...
		if withStrictFixedChar {
			sqlutils.AssertSqlQuery(t, db, "SELECT 2 + ('5' + '2')", []string{"54"})
			sqlutils.AssertSqlQuery(t, db, "SELECT 3 + ('5' + '2') + 9.5", []string{"64.5"})
		}
		...
		sqlutils.AssertSqlQuery(t, db, "SELECT charextract('ABC', 2)", []string{"B"})
		sqlutils.AssertSqlQuery(t, db, "SELECT charextract('A', 2)", []string{""})
		sqlutils.AssertSqlQuery(t, db, "SELECT charextract('A'::char(10), 2)", []string{" "})
		sqlutils.AssertSqlQuery(t, db, "SELECT charextract (charextract('ABC', 2), 1)", []string{"B"})
		sqlutils.AssertSqlQuery(t, db, "SELECT charextract (456, 2)", []string{"5"})
		sqlutils.AssertSqlQuery(t, db, "SELECT POSITION('z', 'Company')", []string{"0"})
		sqlutils.AssertSqlQuery(t, db, "SELECT POSITION('a', 'Company')", []string{"5"})
		sqlutils.AssertSqlQuery(t, db, "SELECT LOCATE('Company', 'z')", []string{"8"})
		sqlutils.AssertSqlQuery(t, db, "SELECT LOCATE('Company', 'a')", []string{"5"})
		sqlutils.AssertSqlQuery(t, db, "SELECT nvl(char('b'), 'a')", []string{"b"})
		sqlutils.AssertSqlQuery(t, db, "SELECT nvl(char(null), 'a')", []string{"a"})
		sqlutils.AssertSqlQuery(t, db, "SELECT length ('é')", []string{"1"})
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French', 'uk', 'English', 'it', 'Italian', 'unknown')", []string{"unknown"}, "de")
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French', 'uk', 'English', 'it', 'Italian')", []string{"Italian"}, "it")
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French', 'uk', 'English', 'unknown')", []string{"French"}, "fr")
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French', 'uk', 'English')", []sql.NullString{{}}, "toto")
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French', 'unknown')", []string{"unknown"}, "it")
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French', 'unknown')", []string{"French"}, "fr")
		sqlutils.AssertSqlQuery(t, db, "SELECT DECODE($1, 'fr', 'French')", []string{"French"}, "fr")
		sqlutils.AssertSqlQuery(t, db, "SELECT date_part('mo', TO_DATE('20170503','YYYYMMDD'))", []int{5})
		sqlutils.AssertSqlQuery(t, db, "SELECT smallint($1) + int($2)", []string{"-3"}, "01", "-4")
		sqlutils.AssertSqlExec(t, db, true, "drop table if exists test_table4", 0)
		sqlutils.AssertSqlExec(t, db, true, "drop table if exists test_table5", 0)
		sqlutils.AssertSqlExec(t, db, true, "drop table if exists test_table6", 0)
		sqlutils.AssertSqlExec(t, db, true, "drop table if exists trt_recepisse", 0)

		sqlutils.AssertSqlExec(t, db, true, "INSERT INTO TABLE1 VALUES ('dummy', '100100')", 1)
		sqlutils.AssertSqlExec(t, db, true, "INSERT INTO TABLE2 VALUES ('dummy'),(NULL)", 2)
		...
		if rows != nil {
			...
			sqlutils.AssertNoError(t, err)
			sqlutils.AssertEquals(t, "column1", columns[0], testQuery)
			sqlutils.AssertEquals(t, "col2", columns[1], testQuery)
			sqlutils.AssertEquals(t, "overr1", columns[2], testQuery)
			sqlutils.AssertEquals(t, "col4", columns[3], testQuery)
		}

		//sqlutils.AssertSqlQuery(t, db, "select no_demande = 0", []string{"0"})
		//sqlutils.AssertSqlQuery(t, db, "select no_demande = 0 FROM TABLE1 where colUMN1 = 'dummy'", []string{"0"})
		...
		if odbcInstalled {
			...
			sqlutils.AssertNoError(t, err)
			sqlutils.AssertEquals(t, true, strings.Contains(result, "1 rows fetched"), query)
		}

		defer os.Remove(TestCopyFile)
		query := "COPY TABLE1 TO '" + TestCopyFile + "'"
		sqlutils.AssertSqlExec(t, db, false, query, 1) // false = pas de TX possible pour un COPY, donc pas de prepare avant l'exec
		if odbcInstalled {
			odbcResult, err := isql(query)
			sqlutils.AssertNoError(t, err)
			sqlutils.AssertEquals(t, true, strings.Contains(odbcResult, "SQLRowCount returns 1"), query)
		}
		sqlutils.AssertSqlExec(t, db, false, "COPY TABLE1 (COLUMN1 = varchar(0)tab, heuremaj) INTO '"+TestCopyFile+"'", 1)
		sqlutils.AssertSqlExec(t, db, false, "COPY TABLE TABLE1 () INTO '"+TestCopyFile+"'", 1)
		tmpTest, err := os.ReadFile(TestCopyFile)
		sqlutils.AssertNoError(t, err)
		sqlutils.AssertEquals(t, "5047434f50590aff0d0a00000000000000000000020000000564756d6d7900000006313030313030ffff", hex.EncodeToString(tmpTest), TestCopyFile)
		sqlutils.AssertSqlExec(t, db, false, "truncate TABLE1 ", 0)
		sqlutils.AssertSqlExec(t, db, false, "COPY TABLE TABLE1 () FROM '"+TestCopyFile+"' with allocation = 4, row_estimate = 1091766", 1)
		sqlutils.AssertSqlExec(t, db, false, "COPY TABLE1 (COLUMN1 = char(0)'%') INTO '"+TestCopyFile+"'", 1)
		sqlutils.AssertSqlExec(t, db, false, "COPY table TABLE1 (COLUMN1 = char(05) colon with null('bouh'), heuremaj = CHAR(6)) INTO '"+TestCopyFile+"'", 1)
		tmpTest, err = os.ReadFile(TestCopyFile)
		sqlutils.AssertNoError(t, err)
		...
		sqlutils.AssertSqlExec(t, db, false, "COPY table TABLE2 (COLUMN2 = colon with null('bouh')) INTO '"+TestCopyFile+"'", 2)
		tmpTest, err = os.ReadFile(TestCopyFile)
		sqlutils.AssertNoError(t, err)
		sqlutils.AssertEquals(t, "dummy     \nbouh\n", string(tmpTest), TestCopyFile)
		...
		sqlutils.AssertSqlQuery(t, db, "select upper(COLUMN1) + COLUMN1 from TABLE1", []string{"DUMMYdummy"})
		sqlutils.AssertSqlQuery(t, db, "select char(COLUMN1, 1) + COLUMN1 from TABLE1", []string{"ddummy"})
		sqlutils.AssertSqlQuery(t, db, "select COLUMN1 +upper(COLUMN1)  from TABLE1", []string{"dummyDUMMY"})
		sqlutils.AssertSqlQuery(t, db, "select COLUMN1 + char('A')  from TABLE1", []string{"dummyA"})
		sqlutils.AssertSqlQuery(t, db, "select COLUMN1 + char('A' + 'B')  from TABLE1", []string{"dummyAB"})
		...
		timeResults2 := sqlutils.AssertSqlRowCount[time.Time](t, db, "select current_date", 1)
		if timeResults != nil && timeResults2 != nil {
			sqlutils.AssertEquals(t, (*timeResults2[0]).Unix(), (*timeResults[0]).Unix(), testQuery)
		}
		sqlutils.AssertSqlQuery(t, db, "select char(date('today'))", []string{now.Format("2006-01-02")})
		sqlutils.AssertSqlQuery(t, db, "select date_part('minutes','now')", []string{now.Format("4")})
		sqlutils.AssertSqlQuery(t, db, "select date_part('minutes','today')", []string{"0"})

		testQuery = "select date('2012-12-01 16:55:15')"
		timeResults = sqlutils.AssertSqlRowCount[time.Time](t, db, testQuery, 1)
		if timeResults != nil {
			sqlutils.AssertEquals(t, 16, timeResults[0].Hour(), testQuery)
			sqlutils.AssertEquals(t, 55, timeResults[0].Minute(), testQuery)
		}
		testQuery = "select date_part('minutes', DATE('23-Oct-1998 12:33'))"
		intResults := sqlutils.AssertSqlRowCount[int](t, db, testQuery, 1)
		if intResults != nil {
			sqlutils.AssertEquals(t, 33, *intResults[0], testQuery)
		}

		testQuery = "select date('today') - 1"
		timeResults = sqlutils.AssertSqlRowCount[time.Time](t, db, testQuery, 1)
		if timeResults != nil {
			sqlutils.AssertEquals(t, now.AddDate(0, 0, -1).Month(), timeResults[0].Month(), testQuery)
			sqlutils.AssertEquals(t, now.AddDate(0, 0, -1).Day(), timeResults[0].Day(), testQuery)
		}
		...
		sqlutils.AssertSqlQuery(t, db, "select decimal('2.45656', 4, 2) + 5", []float64{7.45})
		sqlutils.AssertSqlQuery(t, db, "select 'toto' + 'tata'", []string{"tototata"})
		...
		sqlutils.AssertSqlQuery(t, db, "select trim(char(date_part('yr','now')))+ right(trim('0'+char(date_part('mo','now'))),2)+ right(trim('0'+char(date_part('day','now'))),2)", []string{now.Format("20060102")})
		...
		sqlutils.AssertSqlQuery(t, db, "select to_char(EXTRACT(DAY FROM CURRENT_DATE))", []string{strconv.Itoa(now.Day())})
		...
		timeResults = sqlutils.AssertSqlRowCount[time.Time](t, db, "select Ifnull(null, date('05/25/2022'))", 1)
		if timeResults != nil {
			sqlutils.AssertEquals(t, tm.Unix(), (*timeResults[0]).Unix(), testQuery)
		}
		now = time.Now()
		if timeResults != nil {
			timeResults = sqlutils.AssertSqlRowCount[time.Time](t, db, "select sysdate", 1)
			sqlutils.AssertEquals(t, now.Unix(), (*timeResults[0]).Unix(), testQuery)
		}
		sqlutils.AssertSqlQuery(t, db, "SELECT date_part('year', SYSDATE) + date_part('month', SYSDATE) + date_part('day', SYSDATE)", []int{now.Year() + int(now.Month()) + now.Day()})
		sqlutils.AssertSqlRowCount[string](t, db, "select dbmsinfo('SESSION_ID')", 1)
		sqlutils.AssertSqlRowCount[string](t, db, "select dbmsinfo('DUMMY')", 1)
		sqlutils.AssertSqlQuery(t, db, "select char('456', 2)", []string{"45"})
		sqlutils.AssertSqlQuery(t, db, "select char(456) + vchar('A') + varchar(789)", []string{"456A789"})
		sqlutils.AssertSqlQuery(t, db, "select char(456, 2)", []string{"45"})
		if withStrictFixedChar {
			sqlutils.AssertSqlQuery(t, db, "select char('123456', 10) + 'A'", []string{"123456    A"})
			sqlutils.AssertSqlQuery(t, db, "select 'A' + char('123456', 10) + 'B'", []string{"A123456    B"})
			sqlutils.AssertSqlQuery(t, db, "select right(char('123456', 10), 2)", []string{"          "})
			sqlutils.AssertSqlQuery(t, db, "select left(char('123456', 10), 8)", []string{"123456    "})
		} else {
			sqlutils.AssertSqlQuery(t, db, "select char('123456', 10) + 'A'", []string{"123456A"})
			sqlutils.AssertSqlQuery(t, db, "select 'A' + char('123456', 10) + 'B'", []string{"A123456B"})
			sqlutils.AssertSqlQuery(t, db, "select right(char('123456', 10), 2)", []string{"  "})
			sqlutils.AssertSqlQuery(t, db, "select left(char('123456', 10), 8)", []string{"123456  "})
		}
		sqlutils.AssertSqlQuery(t, db, "select (char('  ',8) = '        ') and (char('  ',8) = '')", []string{"true"})
		sqlutils.AssertSqlQuery(t, db, "select $1 * 100", []string{"326.00"}, "0000000003.26")
		sqlutils.AssertSqlQuery(t, db, "select decimal(101.10, 0, 2)+$1", []string{"104.36"}, "0000000003.26")
		sqlutils.AssertSqlQuery(t, db, "select $1*$2", []string{"6.52"}, "0000000003.26", "2")

		now = time.Now()
		timeResults = sqlutils.AssertSqlRowCount[time.Time](t, db, "select TIMESTAMPADD(HOUR, 1, SYSDATE)", 1)
		if timeResults != nil {
			sqlutils.AssertEquals(t, now.Add(time.Hour).Unix(), (*timeResults[0]).Unix(), testQuery)
		}

		currentHour, err := strconv.Atoi(now.Format("15"))
		sqlutils.AssertNoError(t, err)
		sqlutils.AssertSqlQuery(t, db, "select TO_CHAR(TIMESTAMPADD(HOUR, 1, SYSDATE), 'HH24')", []string{fmt.Sprintf("%02d", (currentHour+1)%24)})
		sqlutils.AssertSqlExec(t, db, true, "modify TABLE1 to btree unique on column1, heuremaj with location=(ii_database), fillfactor = 80, extend = 16, allocation = 4", 0)
		sqlutils.AssertSqlExec(t, db, true, "modify table public.TABLE2 to isam on column2 with fillfactor = 80, extend = 16, page_size = 8192", 0)
		sqlutils.AssertSqlExec(t, db, true, "DROP SEQUENCE IF EXISTS seq_tarif", 0)
		sqlutils.AssertSqlExec(t, db, true, "CREATE SEQUENCE seq_tarif INCREMENT BY 1 MINVALUE 1 MAXVALUE 100000 START 1", 0)
		sqlutils.AssertSqlQuery(t, db, "select seq_tarif.nextval", []int{1})

		sqlutils.AssertSqlQuery(t, db, "select tcom.long_remark from $ingres.iidb_subcomments tcom where tcom.object_name = 'tr_jfac_recep_aaaamm' and tcom.object_owner = 'archive'", []string{})
		...
		for range size {
			...
			sqlutils.AssertSqlExec(t, db, false, "INSERT INTO TABLE2 VALUES('"+val+"')", 1)
		}
		if driver == "pgx" { // it seems copy in extended query mode are not supported by lib/pq
			tx, err := db.Begin()
			sqlutils.AssertNoError(t, err)
			sqlutils.AssertSqlExec(t, tx, false, "COPY TABLE2 INTO '"+TestCopyFile+"'", size)
			err = tx.Commit()
			sqlutils.AssertNoError(t, err)
			...
			tc2, err := os.Stat(TestCopyFile + "2")
			sqlutils.AssertNoError(t, err)
			...
			tx, err = db.Begin()
			sqlutils.AssertNoError(t, err)
			_, err = sqlutils.DoExec(tx, true, "COPY TABLE1 FROM '"+TestCopyFile+"'")
			sqlutils.AssertError(t, err, "ERROR: missing data for column \"heuremaj\" (SQLSTATE 22P04)")
			err = tx.Rollback()
			sqlutils.AssertNoError(t, err)
		} else {
			sqlutils.AssertSqlExec(t, db, false, "COPY TABLE2 INTO '"+TestCopyFile+"'", size)
			sqlutils.AssertSqlExec(t, db, false, "truncate TABLE2 ", 0)
			sqlutils.AssertSqlExec(t, db, false, "COPY TABLE2 FROM '"+TestCopyFile+"'", size)
			sqlutils.AssertSqlExec(t, db, false, "truncate TABLE2 ", 0)

			os.WriteFile(TestCopyFile, []byte("wrong data"), 0644)
			sqlutils.AssertSqlExec(t, db, false, "truncate TABLE1", 0)
			_, err = sqlutils.DoExec(db, false, "COPY TABLE1 FROM '"+TestCopyFile+"'")
			sqlutils.AssertError(t, err, "pq: missing data for column \"heuremaj\" (22P04)")
		}
		log.Printf("time for copy of %d: %d ms", size, time.Since(start).Milliseconds())

		sqlutils.AssertSqlExec(t, db, true, "DROP TABLE TABLE1", 0)
		sqlutils.AssertSqlExec(t, db, true, "DROP TABLE TABLE2", 0)

		if psqlInstalled {
			query := "\\set foo '987'\nSELECT char(:foo, 2);"
			result, err := psql(query)
			sqlutils.AssertNoError(t, err)
			sqlutils.AssertEquals(t, "col1\n\n98\n(1 row)", result, query)
		}
	}
...
ORDER BY h.societe, h.etat, h.agence, h.client`
	...
	ss = time.Now()
	for range numThreads * 100 {
		r, err := sqlutils.ParseSql(large, sqllexer.DBMSOracle)
		sqlutils.AssertNoError(t, err)
		r.Sql()
	}
...
func TestParseFile(t *testing.T) {
	...
	WHERE societe = :societe
	  ...
	  `, sqllexer.DBMSOracle)
	sqlutils.AssertNoError(t, err)
	sqlutils.AssertEquals(t, 3, len(result.Split()))

	result, err = sqlutils.ParseSql("--comment", sqllexer.DBMSOracle)
	sqlutils.AssertNoError(t, err)
	sqlutils.AssertEquals(t, 0, len(result.Split()))
}
